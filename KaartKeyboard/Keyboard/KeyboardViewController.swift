...
//    fileprivate var shiftButton: KeyButton!
    ...
    fileprivate var spaceTitle: String {
        return UserDefaults.standard.string(forKey: "CURRENT_LANG")?.uppercased() ?? "ENGLISH"
    }
    ...
    fileprivate var lastWordTyped: String? {
        if let documentContextBeforeInput = proxy.documentContextBeforeInput as NSString? {
            ...
            if length > 0 {
                let lastChar = documentContextBeforeInput.character(at: length - 1)
                guard let unicodeScalar = UnicodeScalar(lastChar) else {
                    print("KeyboardViewController: Unable to convert character to UnicodeScalar")
                    return nil
                }
...
//        widthConsSpeceButton.isActive = true
    ...
    {
        print("KeyboardViewController: Setting up predictive text constraints")
        
        // Validate predictive text scroll view exists
        guard let predictiveScrollView = predictiveTextScrollView else {
            print("KeyboardViewController: Error - predictiveTextScrollView is nil, cannot set constraints")
            return
        }
        
        // Add Constraints for Predictive Text
        removeAllConstrains(predictiveScrollView);
        
        let topCons = NSLayoutConstraint(item: predictiveScrollView, attribute: .top, relatedBy: .equal, toItem: view, attribute: .top, multiplier: 1.0, constant: spacing);
        
        let rightCons = NSLayoutConstraint(item: predictiveScrollView, attribute: .trailing, relatedBy: .equal, toItem: view, attribute: .trailing, multiplier: 1.0, constant: -spacing );
        
        let heightCons = NSLayoutConstraint(item: predictiveScrollView, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: 30)
        
        let leftCons = NSLayoutConstraint(item: predictiveScrollView, attribute: .leading, relatedBy: .equal, toItem: view, attribute: .leading, multiplier: 1.0, constant: spacing )
        
        predictiveScrollView.translatesAutoresizingMaskIntoConstraints = false;
        ...
        print("KeyboardViewController: Predictive text constraints set successfully")
    }
    ...
    override func viewDidLoad() {
        ...
        print("KeyboardViewController: Adding UI components")
        ...
        print("KeyboardViewController: Requesting supplementary lexicon")
        self.requestSupplementaryLexicon { (lexObj) in
            print("KeyboardViewController: Lexicon received with \(lexObj.entries.count) entries")
            self.lexicon = lexObj;
            
            // Now that lexicon is available, update suggestions if there's already text
    ...
    // MARK: Language Loading Helper Methods
    
    /**
     Validates that a decoded Language object has the required properties for the keyboard to function properly
     */
    private func isLanguageValid(_ language: Language) -> Bool {
        // Check that the language has a title
        guard !language.title.isEmpty else {
            print("Language validation failed: Missing or empty title")
            return false
        }
        
        // Check that the language has rows
        guard !language.rows.isEmpty else {
            print("Language validation failed: No keyboard rows defined")
            return false
        }
        
        // Check that each row has characters
        for (index, row) in language.rows.enumerated() {
            guard !row.row.isEmpty else {
                print("Language validation failed: Row \(index) is empty")
                return false
            }
            
            // Check that each character has at least a primary character
            for (charIndex, char) in row.row.enumerated() {
                guard !char.primary.isEmpty else {
                    print("Language validation failed: Character at row \(index), position \(charIndex) has empty primary character")
                    return false
                }
            }
        }
    ...
    /**
     Creates a minimal fallback English language when all JSON files fail to load
     This ensures the keyboard has at least one working language
     */
    private func createFallbackLanguage() {
        print("Creating fallback English language...")
        
        // Create a basic Language structure with minimal QWERTY layout
        // This is a simplified version that should allow basic typing
        let fallbackLanguage = createBasicEnglishLanguage()
        
        if isLanguageValid(fallbackLanguage) {
            english = fallbackLanguage
            languages.append(fallbackLanguage)
            UserDefaults.standard.set(fallbackLanguage.title, forKey: "CURRENT_LANG")
            print("Fallback English language created successfully")
        } else {
            print("Critical Error: Even fallback language creation failed. Keyboard may not function properly.")
        }
    }
    
    /**
     Creates a basic English Language object programmatically
     */
    private func createBasicEnglishLanguage() -> Language {
        // Define basic QWERTY rows
        let row1Characters = ["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"]
        let row2Characters = ["A", "S", "D", "F", "G", "H", "J", "K", "L"]
        let row3Characters = ["Z", "X", "C", "V", "B", "N", "M"]
        
        // Note: This is a simplified implementation. In a real scenario, you would need to
        // ensure this matches the exact structure expected by your Language model.
        // For now, this serves as a placeholder that demonstrates the concept.
        
        // Since I don't have access to the exact Language model structure,
        // I'll create a basic structure that should work with the existing code
        // You may need to adjust this based on your actual Language model implementation
        
        do {
            // Try to create a minimal JSON structure and decode it
            let fallbackJSON = """
            {
                ...
                "rows": [
                    {
                        "row": [
                            {"primary": "q", "secondary": "1", "tertiary": ["¡", "¿"]},
                            {"primary": "w", "secondary": "2", "tertiary": ["²", "³"]},
                            {"primary": "e", "secondary": "3", "tertiary": ["é", "è", "ê"]},
                            {"primary": "r", "secondary": "4", "tertiary": ["®", "™"]},
                            {"primary": "t", "secondary": "5", "tertiary": ["†", "‡"]},
                            {"primary": "y", "secondary": "6", "tertiary": ["ý", "ÿ"]},
                            {"primary": "u", "secondary": "7", "tertiary": ["ú", "ù", "û"]},
                            {"primary": "i", "secondary": "8", "tertiary": ["í", "ì", "î"]},
                            {"primary": "o", "secondary": "9", "tertiary": ["ó", "ò", "ô"]},
                            {"primary": "p", "secondary": "0", "tertiary": ["¶", "π"]}
                        ]
                    },
                    {
                        "row": [
                            {"primary": "a", "secondary": "!", "tertiary": ["á", "à", "â"]},
                            {"primary": "s", "secondary": "@", "tertiary": ["ß", "§"]},
                            {"primary": "d", "secondary": "#", "tertiary": ["∂", "°"]},
                            {"primary": "f", "secondary": "$", "tertiary": ["ƒ", "£"]},
                            {"primary": "g", "secondary": "%", "tertiary": ["©", "®"]},
                            {"primary": "h", "secondary": "^", "tertiary": ["™", "¡"]},
                            {"primary": "j", "secondary": "&", "tertiary": ["∆", "¿"]},
                            {"primary": "k", "secondary": "*", "tertiary": ["˚", "¢"]},
                            {"primary": "l", "secondary": "(", "tertiary": ["¬", "…"]}
                        ]
                    },
                    {
                        "row": [
                            {"primary": "z", "secondary": ")", "tertiary": ["Ω", "ÿ"]},
                            {"primary": "x", "secondary": "-", "tertiary": ["≈", "µ"]},
                            {"primary": "c", "secondary": "_", "tertiary": ["ç", "©"]},
                            {"primary": "v", "secondary": "=", "tertiary": ["√", "◊"]},
                            {"primary": "b", "secondary": "+", "tertiary": ["∫", "ß"]},
                            {"primary": "n", "secondary": "[", "tertiary": ["ñ", "~"]},
                            {"primary": "m", "secondary": "]", "tertiary": ["µ", "Ó"]}
                        ]
                    }
                ]
            }
    ...
    override func loadView() {
        print("KeyboardViewController: loadView started")
        super.loadView()
        
        print("KeyboardViewController: Loading language preferences")
        _showEnglish = defaults?.bool(forKey: "english") ?? true  // Default to true for English
        _showGreek = defaults?.bool(forKey: "greek") ?? false    // Default to false for other languages
        ...
        for (key, value) in showLanguages {
            ...
            guard let path = bundle.path(forResource: key, ofType: "json") else {
                print("Could not find \(key).json in keyboard extension bundle")
                continue
            }
            ...
            do {
                ...
                do {
                    ...
                    case "english":
                        ...
                        if let validLanguage = english, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                        }
                    case "greek":
                        greek = try JSONDecoder().decode(Language.self, from: data)
                        if let validLanguage = greek, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    case "serbian-cyrillic":
                        ...
                        if let validLanguage = serbian_cyrillic, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    case "romanian":
                        ...
                        if let validLanguage = romanian, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    case "macedonian":
                        ...
                        if let validLanguage = macedonian, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    case "bulgarian":
                        ...
                        if let validLanguage = bulgarian, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    case "vietnamese":
                        ...
                        if let validLanguage = vietnamese, isLanguageValid(validLanguage) {
                            languages.append(validLanguage)
                            print("Successfully loaded language: \(validLanguage.title)")
                        } else {
                            print("Failed to decode \(key).json - decoded language was nil or invalid")
                    ...
                    default:
                        print("Warning: Unrecognized language key: \(key)")
                    }
                } catch let jsonError as DecodingError {
                    print("JSON decoding error for \(key): \(jsonError.localizedDescription)")
                    switch jsonError {
                    ...
                    case .valueNotFound(let type, let context):
                        print("Missing value for type \(type) in \(context.debugDescription)")
                    @unknown default:
                        print("Unknown JSON decoding error")
                    }
                } catch {
                    print("Unexpected error while decoding JSON for \(key): \(error.localizedDescription)")
                }
            } catch {
                print("Error loading file \(key).json: \(error.localizedDescription)")
        ...
        // Ensure we have at least one language loaded before setting the current language
        if !languages.isEmpty {
            UserDefaults.standard.set(languages[0].title, forKey: "CURRENT_LANG")
            print("Loaded \(languages.count) language(s) successfully")
        } else {
            print("Critical Error: No language files were loaded successfully")
            // Attempt to create a fallback English language if no languages loaded
            createFallbackLanguage()
        }
    }
    
    override func viewWillAppear(_ animated: Bool) {
        print("KeyboardViewController: viewWillAppear started")
        super.viewWillAppear(animated)
        
        print("KeyboardViewController: Checking language preferences for changes")
        for (key, value) in showLanguages {
            if value != defaults?.bool(forKey: key) {
                print("KeyboardViewController: Language preference changed for \(key), reloading view")
                self.loadView()
                break
            }
        }
        print("KeyboardViewController: viewWillAppear completed")
    }
    
    override func viewDidAppear(_ animated: Bool) {
        print("KeyboardViewController: viewDidAppear started")
        super.viewDidAppear(animated)
        
        // Validate that essential components are initialized
        if languages.isEmpty {
            print("KeyboardViewController: Critical Error - No languages loaded in viewDidAppear")
        }
        
        if predictiveTextScrollView == nil {
            print("KeyboardViewController: Warning - PredictiveTextScrollView not initialized")
        }
        
        if lexicon == nil {
            print("KeyboardViewController: Warning - Lexicon not available yet")
        }
        
        print("KeyboardViewController: Keyboard components validation complete")
    ...
    @objc func aapButtonPressed(_ sender: KeyButton) {
        ...
        if updateShortField(titleText) == true{
            return
        }
    ...
    func handleSwipeDownForButton(_ button: CharacterButton) {
        ...
        proxy.insertText(charStr)
        if button.secondaryCharacter.count > 1 {
            proxy.insertText(" ")
        }
        updateSuggestions()
    }
    ...
    fileprivate func addPredictiveTextScrollView() {
        print("KeyboardViewController: Creating predictive text scroll view")
        
        // Validate view dimensions
        guard self.view.frame.width > 0 && self.view.frame.height > 0 else {
            print("KeyboardViewController: Warning - view has invalid dimensions, deferring predictive text setup")
            return
        }
        ...
        // Validate the created scroll view
        guard let scrollView = predictiveTextScrollView else {
            print("KeyboardViewController: Error - failed to create predictive text scroll view")
            return
        }
        
        self.view.addSubview(scrollView)
        print("KeyboardViewController: Predictive text scroll view added successfully")
    }
...
//        rowCount = 11.0
    ...
    var selectedShortWordBtn: UIButton?
    ...
    @objc func pasteShortWord(_ gesture:UILongPressGestureRecognizer){
        if gesture.state == .began{
            if let clipboardText = UIPasteboard.general.string {
                self.shortWordTxtFld.text?.append(clipboardText)
            }
        }
    }
    ...
    @objc func doneSelect(_ btn:UIButton){
        ...
        if newStr.isEmpty == false {
            ...
            guard let oldTitle = selectedShortWordBtn?.titleLabel?.text else { return }
...
//        self.updateViewConstraints()
    ...
    fileprivate func updateSuggestions() {
        print("KeyboardViewController: updateSuggestions called")
        
        // Ensure lexicon is available
        guard let lexicon = self.lexicon else {
            print("KeyboardViewController: Lexicon not available, suggestions cannot be updated")
            return
        }
        
        // Ensure predictive text scroll view is available
        guard let predictiveScrollView = self.predictiveTextScrollView else {
            print("KeyboardViewController: PredictiveTextScrollView not available")
            return
        }
        
        if let lastWord = lastWordTyped {
            print("KeyboardViewController: Last word typed: \(lastWord)")
            
            let filtedArray = lexicon.entries.filter({ (lexiconEntry) -> Bool in
            ...
            DispatchQueue.main.async(execute: {
                print("KeyboardViewController: Creating \(filtedArray.count) suggestion buttons")
                
                for view in predictiveScrollView.subviews {
                ...
                for i in 0..<filtedArray.count
                {
                    ...
                    predictiveScrollView.addSubview(suggestionButton)
                    self.suggestionButtons.append(suggestionButton)
                    
                    x += self.predictiveTextButtonWidth + self.spacing
                }
                
                predictiveScrollView.contentSize = CGSize(width: x, height: self.predictiveTextBoxHeight)
            })
        }
    }
}
